...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted treasury address is changed
    event NewTreasury(address oldTreasury, address newTreasury);

    /// @notice Emitted gNFT address is set
    event GNFT(address gNFT);

    /// @notice Emitted TPI address is set
    event TPI(address tpi);
    ...
    /**
     * @notice Sets new treasury which collects and distributes reserves
     * @dev onlyAdmin
     */
    ...
    /**
     * @notice Sets governance gNFT token
     * @dev onlyAdmin and only once
     */
    ...
    /**
     * @notice Sets Tonpound Participation Index token
     * @dev onlyAdmin and only once
     */
    function _setCompAddress(address _compAddress) external {
        require(msg.sender == admin, "Only admin");
        require(compAddress == address(0) && _compAddress != address(0), "Invalid compAddress");

        compAddress = _compAddress;
        emit TPI(_compAddress);
    }
